...
export const questions = [
  ...
  q('239', '数组/窗口', 'Hard', '滑动窗口最大值', '返回每个长度 k 窗口的最大值。', '单调递减双端队列保存下标；队首是最大值，过期下标和不可能成为最大值的尾部下标都要删除。', py('from collections import deque\n\ndef max_sliding_window(nums, k):\n    if not nums or k <= 0 or k > len(nums):\n        return []\n    window = deque()\n    result = []\n    for i, value in enumerate(nums):\n        while window and window[0] <= i - k:\n            window.popleft()\n        while window and nums[window[-1]] <= value:\n            window.pop()\n        window.append(i)\n        if i >= k - 1:\n            result.append(nums[window[0]])\n    return result'), '时间 O(n)，空间 O(k)', {
    beginnerSummary: '窗口每次右移一格，要快速知道窗口最大值。队列下标对应的值从队首到队尾递减，所以队首总是最大候选。',
    prerequisites: ['deque 两端都能 O(1) 加删。', '保存下标而不只是值，才能判断元素是否已经滑出窗口。'],
    workedExample: ['nums=[1,3,-1,-3,5]、k=3：加入 1、3 时从尾删除 1，队首为 3，首个答案是 3。', '加入 5 前清掉过期下标，再从尾删除较小值，窗口 [−3,5] 的最大值为 5。'],
    derivation: ['每个窗口重新 max 需要 O(nk)。', '较小且更晚的值会被更大值永久淘汰；每个下标最多入队出队一次，故线性。'],
  ...
  q('334', '数组/窗口', 'Medium', '递增三元子序列', '判断是否存在 i<j<k 且 nums[i]<nums[j]<nums[k]。', '维护遍历至今最小的 first 和在 first 之后可行的最小 second；遇到更大的数即组成三元组。', py('def increasing_triplet(nums):\n    first = second = float("inf")\n    for value in nums:\n        if value <= first:\n            first = value\n        elif value <= second:\n            second = value\n        else:\n            return True\n    return False'), '时间 O(n)，空间 O(1)', {
    beginnerSummary: '只需知道是否存在，不必保存三元组。first 越小越容易找到第二个数；second 也尽量小，后面更容易出现第三个更大值。',
    prerequisites: ['子序列要求下标递增，但中间允许跳过元素。', '用无穷大表示还没有找到合法的 first 或 second。'],
    workedExample: ['nums=[2,1,5,0,4,6]：先把 first 更新为 1，读到 5 得 second=5。', '读到 0 后把 first 更新得更小，读到 4 把 second 降为 4，最后 6>4，返回 True。'],
    derivation: ['枚举三元组需要 O(n³)，维护所有前缀候选也会浪费空间。', '保留最小的一对 first<second 是安全的：更小的阈值不会减少未来可行的第三个数。'],
    lineByLine: ['value<=first 时更新最小起点。', '否则若不超过 second，更新更小的中间值。', '既大于 first 又大于 second 时，三元组已按扫描顺序成立。', '循环结束仍无第三值则返回 False。'],
    edgeCases: ['长度小于 3 直接返回 False', '严格递增要求使用 <，相等值不能充当下一项', '全相等或严格递减数组没有三元组'],
    followUps: [{ question: '为什么更新 first 不会破坏下标顺序？', answer: 'first 只会被当前更小值替换，second 仍来自它之后的扫描位置；算法只用阈值表示存在性，替换不会制造逆序。' }, { question: '如何返回一组具体下标？', answer: '额外保存 first_index 和 second_index；发现第三个值时返回这两个下标及当前下标。' }],
  }),
  ...
  q('215', '二分/TopK', 'Medium', '数组第 K 大元素', '在无序数组找第 k 大。', '把第 k 大转换为升序下标 n-k，用原地 partition 把目标放到正确一侧，重复缩小范围。', py('def find_kth_largest(nums, k):\n    if not 1 <= k <= len(nums):\n        return None\n    target = len(nums) - k\n\n    def partition(left, right):\n        pivot = nums[right]\n        store = left\n        for i in range(left, right):\n            if nums[i] <= pivot:\n                nums[store], nums[i] = nums[i], nums[store]\n                store += 1\n        nums[store], nums[right] = nums[right], nums[store]\n        return store\n\n    left, right = 0, len(nums) - 1\n    while left <= right:\n        pivot_index = partition(left, right)\n        if pivot_index == target:\n            return nums[pivot_index]\n        if pivot_index < target:\n            left = pivot_index + 1\n        else:\n            right = pivot_index - 1\n    return None'), '平均 O(n)，最坏 O(n²)，空间 O(1)', {
